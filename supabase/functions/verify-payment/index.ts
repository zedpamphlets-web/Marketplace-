// POST { groupId } → check Lipila and mark group + all child orders paid.
import { corsHeaders, requireUser, serviceClient, loadOwnedGroup, loadOwnedOrder, lipilaFetch } from "../_shared/lipila.ts";

Deno.serve(async (req) => {
  if (req.method === "OPTIONS") {
    return new Response("ok", { headers: corsHeaders });
  }

  try {
    const userId = await requireUser(req);
    if (!userId) {
      return json({ error: "Not signed in." }, 401);
    }

    const body = await req.json();
    const groupId = body.groupId || body.orderId;
    if (!groupId) {
      return json({ error: "Missing groupId." }, 400);
    }

    const db = serviceClient();
    const group = await loadOwnedGroup(db, groupId, userId);

    if (group) {
      if (group.payment_status === "paid") {
        return json({ status: "paid" });
      }

      const { ok, json: lipilaBody } = await lipilaFetch(
        `/collections/check-status?referenceId=${encodeURIComponent(group.id)}`,
        null
      );
      if (!ok) {
        return json({ status: "pending" });
      }

      const providerStatus = String(lipilaBody?.status || "").toLowerCase();
      if (providerStatus === "successful" || providerStatus === "success") {
        await db.from("order_groups").update({ payment_status: "paid" }).eq("id", group.id);
        await db.from("orders").update({ payment_status: "paid" }).eq("group_id", group.id);
        return json({ status: "paid" });
      }
      if (providerStatus === "failed") {
        await db.from("order_groups").update({ payment_status: "failed" }).eq("id", group.id);
        await db.from("orders").update({ payment_status: "failed" }).eq("group_id", group.id);
        return json({ status: "failed", reason: lipilaBody?.message });
      }
      return json({ status: "pending" });
    }

    // Legacy single-order path
    const order = await loadOwnedOrder(db, groupId, userId);
    if (!order) {
      return json({ error: "Order not found." }, 404);
    }
    if (order.payment_status === "paid") {
      return json({ status: "paid" });
    }

    const { ok, json: lipilaBody } = await lipilaFetch(
      `/collections/check-status?referenceId=${encodeURIComponent(order.id)}`,
      null
    );
    if (!ok) return json({ status: "pending" });

    const providerStatus = String(lipilaBody?.status || "").toLowerCase();
    if (providerStatus === "successful" || providerStatus === "success") {
      await db.from("orders").update({ payment_status: "paid" }).eq("id", order.id);
      return json({ status: "paid" });
    }
    if (providerStatus === "failed") {
      await db.from("orders").update({ payment_status: "failed" }).eq("id", order.id);
      return json({ status: "failed", reason: lipilaBody?.message });
    }
    return json({ status: "pending" });
  } catch (e) {
    return json({ error: e instanceof Error ? e.message : "Unexpected error." }, 500);
  }
});

function json(body: unknown, status = 200) {
  return new Response(JSON.stringify(body), {
    status,
    headers: { ...corsHeaders, "Content-Type": "application/json" },
  });
}
